...
表现：测试中的行为已经正确，但断言从公开投影读取只存在于 World 内部的状态字段，得到 `null` 并使正式测试退出失败。

处理方式：先确认失败字段是否属于生产入口的公开返回契约；如果只是测试用的内部状态，应从测试 World 的内部居民记录读取，并另外保留公开投影的行为断言。不要为了让断言通过而把内部调试字段泄露到生产接口。
...
直接原因：测试通过 `get_resident_state` 的公开投影读取 `decisionPending`，但该字段只存在于 World 内部居民记录，不属于生产入口的公开返回值。

修复：保留生产接口边界不变，将测试断言改为读取测试 World 的内部居民状态；同步补充对话优先级回归覆盖。
