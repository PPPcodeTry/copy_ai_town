...
- 修复 macOS 版选择 15 名完全自定义居民后可能无法创建小镇的问题；居民现在不再需要匹配任何内置人物或固定商铺负责人。
- 职业与工作地点统一按当前入镇居民安排：推荐组合会根据当前候选居民尽量覆盖不同职业，同一职业可以有多人，职业暂时空缺也不会阻止建镇；居民选择页会提示空缺职业，相关服务会等待有居民任职后恢复。
- 食堂、维修和商铺服务不再依赖指定的内置居民；自定义居民担任对应职业后可以正常接手工作。
- 旧存档中的商铺负责人信息仍可正常读取，并会在后续保存时自动整理为新的职业安排方式。
- 修复自定义居民的内心观察头像可能显示成内置居民形象，以及部分自定义居民旧记忆标识直接显示在界面上的问题。
- 修复同名居民进行内心观察时可能显示另一位居民实时外观的问题；现在会按居民身份准确读取资料。
- 食堂没有主理人或可用帮工时，很饿的居民现在可以进入公共食堂自行做简餐；这条食物路径在多日运行和保存恢复后仍然有效，已经吃完的简餐不会重复结算。
- 食堂兼职帮工现在能够实际完成备餐和清理，同时保留原来的正式职业，也不会被当成食堂负责人或获得未授权的服务权限。
- 诊所没有医生或药师时，轻伤和普通不适的居民现在仍可进入诊所休息并完成基础自我处理；正式问诊和检查继续要求具备诊疗资格的居民，普通帮工不会被当成医生。
- 没有合格医生时，等待过久的重伤居民会暂时离镇接受医疗援助，治疗完成后从南入口返回并继续恢复；离镇、治疗和返回进度可以正常保存和读取，不会让重伤永久停在等待治疗状态。
- 咖啡馆没有咖啡师时，居民仍可进入并使用公共座位休息，但不能继续点单或取用点心；图书馆没有管理员时仍可现场阅读和写作，借还书和馆员协助会等有人任职后恢复。
- 公共场所的帮工不会被当成正式负责人，也不会因此获得借还书等专业权限；无人值守状态在保存和读取后会继续按实际在岗居民生效。
- 工作坊维修、镇务办理、咖啡点单、借还书、商品销售、演出和植物研究在对应职业无人到岗时会明确暂停；居民重复尝试不再生成无人处理的待办，有具备相应能力的居民到岗后会自然恢复。
- 新建小镇现在可以选择 1～15 位居民，默认推荐仍为 15 位；没有使用的住宅会保持空置，模型分配页也只显示本局实际选择的居民。
- 少人口开局会在确认名单前直接说明空缺职业以及对应服务会降级运行还是暂停；关键职业空缺不会阻止创建小镇。
- 修复少人口小镇因空置住宅没有主人而无法连接居民模型或读取存档的问题；创建、保存、继续游戏和再次保存后都会保持本局原有居民集合，局内批量修改居民模型时也只显示实际居民。
- 居民死亡后的新居民补位现在以本局原有人口为目标；例如五人小镇只会恢复到五名在世居民，不会继续补到 15 名，保存和读取后也会保持同一目标。
