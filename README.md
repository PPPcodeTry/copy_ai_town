...
- beta1 至 beta5 的小镇存档可自动升级为 beta6，并保留原修订；beta6 存档可直接继续游戏。
- 最新存档损坏时，会在 World 与 Agent 完整配对可验证的前提下恢复并发布新修订，不再覆盖原档。
- 保存或恢复意外中断后，可以根据事务日志完成安全协调；证据不足时只提供诊断导出，不会猜测进度或建议删除存档。
- 新增 Windows 超长路径回归，检查中断事务、重读和清理。
- 修复坏档恢复时模型服务不可用导致无法进入小镇的问题；恢复完成后会提示玩家到暂停菜单的“模型设置”更换模型。
