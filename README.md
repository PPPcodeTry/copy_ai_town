...
这次更新支持连接其他电脑或局域网设备上的模型服务，并在使用未加密的远程 HTTP 地址前明确说明传输风险；公告会显示发布者身份，玩家化身与居民之间会记录熟悉程度和好感变化；减少居民较多或三倍速运行时的集中卡顿、视觉瞬移和对话切页短暂空白；同时清理了居民功能气泡外的淡色矩形边框，并让测试版在游戏各页右下角持续显示当前版本，方便玩家截图反馈和区分不同安装包；打开源码工程时会更早发现缺失的资源引用，减少启动前的解析报错；离开小镇时，后台已经准备好的居民留言会随存档保留，并在下次继续游戏时显示。
