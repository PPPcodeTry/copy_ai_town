...
- 修复更换模型服务或 API Key 后旧小镇存档无法继续的问题；游戏会自动检查当前可用模型，尽量迁移失效的居民模型绑定，不会把 API Key 写进存档，也不会因为连接失败删除原存档。
- 修复手机版居民模型分配页面被降级的问题；横屏手机现在使用正式页面并按屏幕比例适配，模型连接发生变化时会自动尝试切换到当前可用模型，失败时也会保留草稿并显示明确原因。
- 修复模型自动恢复在正式启动检查阶段无法加载的问题，旧存档恢复流程现在可以正常进入后续验证。
