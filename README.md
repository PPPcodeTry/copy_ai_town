...
这次更新支持连接其他电脑或局域网设备上的模型服务，并在使用未加密的远程 HTTP 地址前明确说明传输风险；减少居民较多或三倍速运行时的集中卡顿、视觉瞬移和对话切页短暂空白；同时完善公共食堂的统一备餐、菜单公告和四人同时用餐规则，让满员居民能够自行回应并稍后再来；居民饿到阈值后也会及时重新判断是否去吃饭；同时清理了居民功能气泡外的淡色矩形边框，并让测试版在游戏各页右下角持续显示当前版本，方便玩家截图反馈和区分不同安装包。
