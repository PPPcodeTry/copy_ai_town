...
- 加载游戏页现在会在旧存档无法进入时保留“更改居民模型”入口；改绑使用暂停页同一套居民模型分配界面，保存成功后生成新的完整修订，原存档不会被覆盖。
- 窄屏横屏和竖屏下重新整理存档卡片与操作按钮，恢复信息、删除和改绑按钮不会互相遮挡；较新版本存档会明确显示为只能查看。
- 加载游戏页的垃圾桶和居民改绑操作现在沿存档纸张右侧上下排列；改绑入口按原方向复用正式人物齿轮扩展牌，并使用固定文字图确保位置稳定。
- 修复 1024×576 和 1280×720 桌面窗口下居民模型分配页误用放大备用布局、导致界面过大和右侧内容出屏的问题；横屏实机现在统一使用暂停页的正式美术界面。
