...
这次更新让 MiniMax 官方接入支持 MiniMax M3 图片输入，并让 M3 的居民决策默认关闭思考，优先快速返回结果；同时集中改善小镇头顶对话、聊天消息、公开想法、居民动作文字和操作提示的断行与分页，让短句、长句和中英混排内容都能完整、均衡地显示，不再出现单字尾行、单字气泡或静默裁字；并修复人物进入建筑、树木等前景区域时，局部画面会突然变暗的问题。
